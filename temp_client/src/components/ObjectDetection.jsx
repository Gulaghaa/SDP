...
const WEBSOCKET_URL = "wss://smart-inventory-management-k5rx.onrender.com/stream";
        ...
        socketRef.current.onmessage = (event) => {
            ...
            if (data.detections) {
                ...
                data.detections.forEach((d) => {
                    console.log(`🛑 ${d.label} | Confidence: ${(d.confidence * 100).toFixed(2)}%`);

                    // Check if one of the detected items matches our searched object
                    if (d.label.toLowerCase() === itemName.toLowerCase() && d.confidence > 0.6) {
                        console.log(`✅ Matched Item: ${d.label} with Confidence: ${(d.confidence * 100).toFixed(2)}%`);
                        found = true;
                    }
                });
